...
class BCICompIV2a:
    def __init__(self, root, args, sub, memory, sub_id, train=True):
        ...
        Args:
            ...
            args (Namespace): Arguments object with n_classes attribute.
        ...
        raw_data = np.transpose(trainX, (2, 1, 0))  # Rearrange dimensions
        ...
        if args.n_classes == 2:
            ...
            raise ValueError("Unsupported class type. Only 2 and 4 classes are supported.")
...
class IncrementalDataStreaming(object):
    ...
    def load_data(self, sub_id):
        ...
        self.train_set[sub_id] = BCICompIV2a(root=self.root, args=self.args, sub=self.subject_queue[sub_id], memory=memory,
                                             sub_id=sub_id)
        ...
        test_loader = torch.utils.data.DataLoader(
            self.test_set[sub_id], batch_size=int(self.batch_size),
            shuffle=True, num_workers=self.n_workers,
            pin_memory=self.pin_memory, drop_last=True
        )
        ...
        self.dataloaders[sub_id]['name'] = f'BCI-COMP2A-subject{self.subject_queue[sub_id]}'
