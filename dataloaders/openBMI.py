...
class openBMI:
    def __init__(self, root, sub, memory, sub_id, train=True):
        ...
        self.data = np.array(data_dict['X'])
        self.targets = np.array(data_dict['Y']).squeeze().tolist()
...
class IncrementalDataStreaming(object):
    """docstring for IncrementalDataStreaming"""
    ...
    def load_data(self, sub_id):
        ...
        self.train_set[sub_id] = openBMI(root=self.root, sub=self.subject_queue[sub_id], memory=memory, sub_id=sub_id)
        ...
        self.dataloaders[sub_id]['name'] = f'OpenBMI-subject{self.subject_queue[sub_id]}'
